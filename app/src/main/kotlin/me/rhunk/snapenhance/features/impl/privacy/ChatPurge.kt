...
class ChatPurge : Feature("ChatPurge", loadParams = FeatureLoadParams.ACTIVITY_CREATE_ASYNC) {
    ...
    private fun deleteMessage(conversationId: SnapUUID, message: Message) {
        ...
        try {
            if (messageLogger.isMessageRemoved(messageId)) {
                return
            }
            ...
            runCatching {
                ...
                Logger.xposedLog("Error deleting message $messageId", throwable)
            }

            // Delay between deletes
            val autoPurgeSpeed = try {
                ConfigProperty.AUTO_PURGE_SPEED.toString().toLong()
            } catch (e: NumberFormatException) {
                Logger.xposedLog("The string is not a valid long integer.")
                1L // Default to 1 second
            }

            Thread.sleep(autoPurgeSpeed * 1000)
        } catch (e: Exception) {
            Logger.xposedLog("An error occurred while processing the message deletion", e)
        }
    }

    private fun canDeleteMessage(message: Message): Boolean {
        //Make sure it's our userId or obv we can't delete
        if (message.senderId.toString() != myUserId) return false

        //Make sure if we support the content type if not we return false as we can't delete
        if (message.messageContent.contentType != ContentType.CHAT &&
            message.messageContent.contentType != ContentType.NOTE &&
            message.messageContent.contentType != ContentType.SNAP &&
            message.messageContent.contentType != ContentType.STICKER &&
    ...
    private fun fetchMessagesPaginated(conversationId: String, lastMessageId: Long, onResult: (Result<List<Message>>) -> Unit) {
        try {
            val callback = CallbackBuilder(fetchConversationWithMessagesCallbackClass)
                .override("onFetchConversationWithMessagesComplete") { param ->
                    try {
                        ...
                        Logger.xposedLog("Error processing fetched messages", e)
                        onResult(Result.failure(Exception("Failed to process fetched messages")))
                    }
                }
                .override("onServerRequest", shouldUnhook = false) {}
                .override("onError") {
                    Logger.xposedLog("Error fetching messages")
            ...
            Logger.xposedLog("An error occurred while fetching messages", e)
            onResult(Result.failure(Exception("Failed to fetch messages")))
        }
    }
    ...
    private fun fetchMessagesRecursively(conversationId: SnapUUID, lastMessageId: Long) {
        try {
                    ...
                    if (!messages.isNullOrEmpty()) {
                        messages.forEach {
                            try {
                                ...
                                Logger.xposedLog("Error processing message deletion", e)
                            }
                        }
            ...
            Logger.xposedLog("An error occurred while fetching messages recursively", e)
        }
    }
